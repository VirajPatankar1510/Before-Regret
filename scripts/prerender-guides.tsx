...
async function run() {
  ...
  // The seven research studies. They are static HTML built by prerender-research.tsx rather than
  // articles rows, so they cannot be read out of the database like the guides above -- which is
  // exactly why llms.txt had never listed a single one. The file that exists specifically to tell
  // an answer engine what this site holds was omitting the only original data on it.
  //
  // Each entry names the figures a model would need in order to quote the study correctly, and the
  // machine-readable files, because a CSV URL is worth more to a retrieval pipeline than prose is.
  // Asserted against dist below rather than trusted: a hand-typed slug that stops matching a built
  // page would advertise a 404 to every crawler that reads this file.
  const RESEARCH: Array<{ slug: string; title: string; summary: string; data: string[] }> = [
    { slug: 'risk-without-price', title: 'Risk Without Price',
      summary: 'What 50.7 million mortgaged US households report paying to insure their homes, against FEMA modelled hazard loss, across 3,093 counties. Which state a home sits in explains more of the price (R2 0.44) than the modelled risk it faces (R2 0.20). 76.5% of modelled building loss falls outside a standard policy.',
      data: ['risk-without-price-by-state.csv', 'risk-without-price-figures.json'] },
    { slug: 'risk-without-cover', title: 'Risk Without Cover',
      summary: 'NFIP flood insurance take-up inside FEMA-mapped flood zones, 2,304 counties and 3.75 million homes. Median county take-up is 15%; coverage tracks state far more closely than modelled flood risk.',
      data: ['flood-takeup-by-county.csv', 'flood-takeup.json'] },
    { slug: 'outside-the-zone', title: 'Outside the Zone',
      summary: '2,578,413 NFIP flood claims from the 1970s to 2026. 26.6% were paid on policies rated outside the high-risk flood zone, totalling $20.6 billion. Zone is as rated at time of claim, not position on today map.',
      data: ['outside-the-zone-by-county.csv', 'flood-outside-zone.json'] },
    { slug: 'high-hazard-dams', title: 'High-Hazard Dams by County',
      summary: 'The 17,049 dams the US Army Corps of Engineers classifies high hazard potential, summarised by county. 16.4% carry a recorded condition of poor or unsatisfactory; 636 of those also have no emergency action plan on file; their median completion year is 1955.',
      data: ['high-hazard-dams-by-county.csv', 'dams-high-hazard.json'] },
    { slug: 'allegheny-storm-premium', title: 'Third Stormiest, Sixth Cheapest',
      summary: 'Storm frequency, housing age and reported homeowners insurance cost across 100 large US counties, centred on Allegheny County, Pennsylvania. NOAA storm events 2015-2024 against ACS B25141 reported insurance cost.',
      data: ['storm-and-premium-counties.csv', 'storm-and-premium-figures.json'] },
    { slug: 'north-texas-roof-age', title: 'Built Together, Due Together',
      summary: 'Housing construction-era cohorts against severe-weather exposure across 100 US counties, showing where large numbers of roofs reach end of life in the same few years.',
      data: ['storm-and-premium-counties.csv', 'storm-and-premium-figures.json'] },
  ...
  // every study page is built from, so its presence is the same guarantee one step earlier.
  //
  // The DATA files cannot be checked here at all -- four of the thirteen are generated by
  // prerender-research itself. That check lives at the end of that script, which reads the llms.txt
  // this one wrote and verifies every research URL in it resolves to a built file.
  for (const r of RESEARCH) {
    if (!fs.existsSync(path.join(process.cwd(), 'docs', `${r.slug}.html`))) {
  ...
  // county pages were retired (see src/data/legacyUrls.ts) while the template kept emitting their
  // headings. An empty section tells a reader the site has a county section that is broken, which is
