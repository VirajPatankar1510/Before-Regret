...
// -------------------------------------------------------------------------------------------
// The county lookup. It exists for reporters more than for readers: an editor will strip an inline
// URL out of copy and will not strip the credit line inside a widget they have already embedded,
// so the embed is the link most likely to survive a copy edit. That is the whole point of pitching.
const lookupData = counties.map((c) => ({
  ...
  r: c.radonZone,
  t: Object.entries(c.eventsByType).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([k, v]) => `${k} ${v}`).join(' &middot; '),
...
const LOOKUP_MARKUP = `<div class="lookup">
    <p class="lk-eyebrow">Look up any of the ${counties.length} counties</p>
    <label class="sr" for="lkpick" style="position:absolute;left:-9999px">Choose a county</label>
...
const DATA_SCRIPT = `<script type="application/json" id="alg-data">${JSON.stringify(lookupData)}</script>`;

// No framework, no build step, no external request: it has to run inside someone else's page.
...
// noindex because it is a duplicate of content that belongs to the study page, and the credit line
// is not optional -- it is the reason the embed exists.
...
<style>
  *{box-sizing:border-box}
  ...
  .lookup{max-width:680px;margin:0 auto;border:0;padding:0}${LOOKUP_CSS}
  .lookup{border:1px solid #e5e0d8;padding:1.1rem 1.2rem}
...
<body>
${LOOKUP_MARKUP}
<p class="embed-credit"><a href="${STUDY_URL}" target="_blank" rel="noopener">Third Stormiest, Sixth Cheapest</a> &mdash; Before Regret</p>
${DATA_SCRIPT}
${JS_SCRIPT}
<script>
