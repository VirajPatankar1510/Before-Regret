...
${newsroomBlock({ title: 'Risk Without Price', url: `${escapeHtmlAttr(CANONICAL_URL)}`, source: 'Census ACS &#183; FEMA National Risk Index', published: '30 August 2026',
  sentence: 'Across 3,093 US counties, modelled hazard explains part of what homeowners pay for insurance, but two counties facing the same modelled risk can differ by more than double, according to an analysis of Census and FEMA National Risk Index data by Before Regret.',
...
${newsroomBlock({ title: 'Risk Without Cover', url: `${escapeHtmlAttr(COVER_URL)}`, source: 'FEMA NFIP &#183; Census ACS', published: '31 August 2026',
  sentence: 'In the typical US county, roughly one home in seven inside a mapped flood zone carries an NFIP flood policy, and take-up does not track flood risk, according to an analysis of FEMA and Census data by Before Regret.',
...
${newsroomBlock({ title: 'Outside the Zone', url: `${escapeHtmlAttr(ZONE_URL)}`, source: 'FEMA NFIP claims &#183; 1,921 counties', published: '1 September 2026',
  sentence: 'More than one paid NFIP flood claim in four came from outside the mapped high-risk flood zone, and in Texas it was about half, according to an analysis of FEMA claims data by Before Regret.',
...
${newsroomBlock({ title: 'High-Hazard Dams by County', url: `${escapeHtmlAttr(DAM_URL)}`, source: 'USACE National Inventory of Dams', published: '1 September 2026',
  sentence: 'Of 17,049 US dams classified high hazard potential, about one in six carries a condition rating of poor or unsatisfactory, and 636 of those have no emergency action plan on file, according to an analysis of the National Inventory of Dams by Before Regret.',
...
${ANALYTICS_BEACON}
  ...
  // different argument -- otherwise it is the county pages again with better typography.
  //
  // docs/raise-or-remove.html is GENERATED by scripts/build-raise-or-remove-study.ts, same
  // contract as outside-the-zone.html and high-hazard-dams.html: prose lives in the builder, every
  // figure is interpolated from docs/data/storm-and-premium-figures.json, and the builder asserts
  // its own headline ("third stormiest, sixth cheapest") against the data before it will write.
  //
  ...
  // four carry a lookup because an embed's credit line survives a copy edit where an inline URL
  // often does not; this one should get the same treatment before it is pitched widely.
  const RR_SRC = path.join(process.cwd(), 'docs', 'raise-or-remove.html');
  if (fs.existsSync(RR_SRC)) {
    const rrSource = fs.readFileSync(RR_SRC, 'utf8');
    const rWrap = rrSource.indexOf('<div class="wrap">');
    if (rWrap === -1) {
      console.error('[prerender-research] raise-or-remove.html has no document body.');
      process.exit(1);
    }
    ...
    if (RR_TITLE.length > 60) { console.error(`[prerender-research] RR_TITLE is ${RR_TITLE.length} chars, over the 60-char display budget.`); process.exit(1); }
    if (RR_DESC.length > 155) { console.error(`[prerender-research] RR_DESC is ${RR_DESC.length} chars, over 155.`); process.exit(1); }

    const RR_LD = [
      {
        '@context': 'https://schema.org',
        '@type': 'ScholarlyArticle',
        headline: 'Raise or Remove',
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>${escapeHtmlAttr(RR_TITLE)}</title>
  <meta name="description" content="${escapeHtmlAttr(RR_DESC)}">
  <meta name="robots" content="index, follow, max-image-preview:large, max-snippet:-1">
  <link rel="canonical" href="${escapeHtmlAttr(RR_URL)}">
  <link rel="icon" href="/favicon.svg" type="image/svg+xml">
  <meta property="og:type" content="article">
  <meta property="og:site_name" content="Before Regret">
  <meta property="og:url" content="${escapeHtmlAttr(RR_URL)}">
  <meta property="og:title" content="${escapeHtmlAttr(RR_TITLE)}">
  <meta property="og:description" content="${escapeHtmlAttr(RR_DESC)}">
  <meta property="og:image" content="${escapeHtmlAttr(OG_IMAGE)}">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="${escapeHtmlAttr(RR_TITLE)}">
  <meta name="twitter:description" content="${escapeHtmlAttr(RR_DESC)}">
  <meta name="twitter:image" content="${escapeHtmlAttr(OG_IMAGE)}">
${rHead.trim()}
  <style>${EXTRA_CSS}</style>
${SITE_ENTITY_LD}
...
${ANALYTICS_BEACON}
    ...
    const rrDir = path.join(process.cwd(), 'dist', 'research', 'raise-or-remove');
    fs.mkdirSync(rrDir, { recursive: true });
    fs.writeFileSync(path.join(rrDir, 'index.html'), rrHtml, 'utf8');
    console.log(`[prerender-research] Wrote static HTML for /research/raise-or-remove/ (${Math.round(rrHtml.length / 1024)} KB)`);

    // ---- /research/raise-or-remove/embed/ ---------------------------------------------
    // Unlike the other four embeds, this one is not sliced back out of the study document. Those
    // extract markup and CSS with indexOf() against the study's exact internals (".lk-eyebrow{",
    // "</header>", ".pull{"), so a cosmetic edit upstream can ship an empty iframe onto someone
    // else's site -- a failure nobody would report to us. build-raise-or-remove-study.ts emits the widget
    // once, from the same data as the page, and this copies the finished file.
    // This study brings its own data, unlike the two regional pieces which share the storm file.
    const rrDataDir = path.join(process.cwd(), 'dist', 'research', 'data');
    fs.mkdirSync(rrDataDir, { recursive: true });
    for (const file of ['raise-or-remove-states.csv', 'raise-or-remove-zips.csv', 'raise-or-remove-figures.json']) {
      const src = path.join(process.cwd(), 'docs', 'data', file);
      if (!fs.existsSync(src)) {
        console.error(`[prerender-research] ${file} missing -- run scripts/analyse-raise-or-remove.ts.`);
        process.exit(1);
      }
      fs.copyFileSync(src, path.join(rrDataDir, file));
    }
    console.log('[prerender-research] Published raise-or-remove state, ZIP and figures files');

    const RR_EMBED_SRC = path.join(process.cwd(), 'docs', 'raise-or-remove.embed.html');
    if (!fs.existsSync(RR_EMBED_SRC)) {
      console.error('[prerender-research] raise-or-remove embed missing -- run scripts/build-raise-or-remove-study.ts.');
      process.exit(1);
    }
    const rrEmbed = fs.readFileSync(RR_EMBED_SRC, 'utf8');
    // The caveat and the credit are the two things that must never be lost in transit: one keeps a
    // count of reports from reading as a hazard ranking, the other is why the embed exists at all.
    for (const required of ['lk-caveat', 'embed-credit', 'rr-data']) {
      if (!rrEmbed.includes(required)) {
        console.error(`[prerender-research] raise-or-remove embed is missing ${required}; refusing to build.`);
        process.exit(1);
      }
    }
    const rrEmbedDir = path.join(rrDir, 'embed');
    fs.mkdirSync(rrEmbedDir, { recursive: true });
    fs.writeFileSync(path.join(rrEmbedDir, 'index.html'), rrEmbed, 'utf8');
    console.log(`[prerender-research] Wrote /research/raise-or-remove/embed/ (${Math.round(rrEmbed.length / 1024)} KB)`);
  }
...
${ANALYTICS_BEACON}
  ...
  const STUDIES: Array<{ url: string; title: string; standfirst: string; finding: string; source: string; data: string[]; embed: boolean; published: string; }> = [
    ...
    {
      url: '/research/raise-or-remove/',
      title: 'Raise or Remove',
      standfirst: 'What the United States actually does with a flooded house, across 32,779 federally funded decisions.',
      finding: 'Demolition beats elevation 2.7 to 1. Five states do nearly all the raising, and only in those five does the foundation under the house change the outcome.',
      source: 'FEMA Hazard Mitigation Assistance &middot; 1996&ndash;2025',
