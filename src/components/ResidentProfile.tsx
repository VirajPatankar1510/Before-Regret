...
import { ChevronLeft, Clock, ShieldCheck, Check, ArrowRight, MessageSquare, Heart, Award, Star, ThumbsUp } from 'lucide-react';
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-gradient-to-tr from-slate-900 via-slate-800 to-blue-950 rounded-2xl p-6 sm:p-8 text-white relative overflow-hidden shadow-xs">
            ...
            <div className="relative z-10">
              ...
              {/* Maintenance Metrics Snapshot */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-8 pt-6 border-t border-white/10 text-xs">
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase font-mono tracking-wider">Water Dependency</p>
                  <p className="font-bold text-white mt-0.5 font-mono text-sm">
                    {locality.waterTankerDependency}% Tankers
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase font-mono tracking-wider">Parking Issues</p>
                  <p className="font-bold text-white mt-0.5 font-mono text-sm">
                    {locality.parkingDisputes >= 50 ? 'Severe Restrictions' : 'Standard Rules'}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-400 font-bold uppercase font-mono tracking-wider">Power Cuts</p>
                  <p className="font-bold text-white mt-0.5 font-mono text-sm">
                    {locality.powerCutHistory} hrs / month
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white border border-slate-200/80 rounded-2xl p-6 sm:p-8">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 pb-6 border-b border-slate-100 mb-6">
              <img
                src={`https://api.dicebear.com/7.x/adventurer/svg?seed=${expert.fullName}&backgroundColor=b6e3f4`}
                alt={expert.fullName}
                className="w-16 h-16 rounded-full object-cover bg-slate-50 border border-slate-200 p-1 shadow-2xs"
                referrerPolicy="no-referrer"
              />
              <div className="text-center sm:text-left flex-1">
                <h4 className="font-bold text-slate-900 text-base">{expert.fullName}</h4>
                <p className="text-xs text-slate-500 mt-0.5">
                  Residing in {locality.name} since {expert.memberSince.split(' ')[1] || '2016'} ({expert.yearsLivingThere} years)
                </p>
          ...
          <div className="space-y-6">
            ...
            {expertReviews.length > 0 ? (
              ...
              <p className="text-xs text-slate-400 italic bg-slate-50 p-6 rounded-2xl text-center border border-dashed border-slate-200">
                No customer consult reviews posted for {expert.fullName} yet. Be the first to ask!
              </p>
            )}
          </div>
