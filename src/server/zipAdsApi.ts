...
export function registerZipAdsRoutes(app: Express) {
  ...
  app.post('/api/zip-ads/checkout', requireVerifiedUser, async (req: Request, res: Response) => {
    ...
    try {
      ...
      const paypalOrder = await createPayPalOrder({
        ...
        description: `Before Regret ZIP ad -- ${tradeCategory} in ${uniqueZips.join(', ')} x 30 days`,
  ...
  app.post('/api/zip-ads/renew/:orderId', requireVerifiedUser, async (req: Request, res: Response) => {
    ...
    try {
      ...
      // website -- all rows in a bundle carry identical values, since they're only ever
      ...
      const paypalOrder = await createPayPalOrder({
        ...
        currency: 'USD',
        type: 'vendor_subscription',
        description: `Before Regret ZIP ad renewal -- ${SLOT_DURATION_DAYS} more days`,
